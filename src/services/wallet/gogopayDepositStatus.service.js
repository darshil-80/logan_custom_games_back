...
/**
 * This service is use to gogopay deposit amount Status
 * @class GetGogopayDepositStatusService
 * @extends {ServiceBase}
 */

export default class GetGogopayDepositStatusService extends ServiceBase {
  async run () {
    const {
      dbModels: {
        User: UserModel,
    ...
    try {
      ...
      try {
        const depositStatus = await axios.get(`${GOGOPAY_URL}/v1/payment/query`, {
          headers: {
            token: authorization
          },
        ...
        const response = depositStatus.data
