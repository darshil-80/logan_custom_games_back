...
/**
 * @export
 * @class CreateCreditTransactionService
 * @extends {ServiceBase}
 */
export default class CreateCreditTransactionService extends ServiceBase {
  // get constraints() {
  //   return constraints
  // }
  ...
  async run () {
    ...
    try {
      ...
      transactionData = await TransactionModel.create({
        ...
        amount: +betData.winningAmount,
        betId: betData.id,
        gameId,
        transactionType: TRANSACTION_TYPES.WIN,
        success: true,
        paymentMethod: PAYMENT_METHODS.GAME,
        transactionId: `${PAYMENT_METHODS.GAME}-${TRANSACTION_TYPES.WIN}-${gameId}-${betData.id}`,
